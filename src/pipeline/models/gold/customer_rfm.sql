...
{% if is_incremental() %}
    WITH rfm_data AS (
        SELECT 
            fs.customer_sk,
            DATE_DIFF('day', MAX(CAST(DATE_PARSE(CAST(fs.order_date_key AS VARCHAR), '%Y%m%d') AS DATE)), CURRENT_DATE) AS recency,
            COUNT(DISTINCT fs.order_id) AS frequency,
            SUM(fs.sales) AS monetary,
            MAX(c.sales_per_customer) AS sales_per_customer,
            SUM(fs.sales) / NULLIF(COUNT(DISTINCT fs.order_id), 0) AS avg_order_value,
    ...
    SELECT 
        ...
        customer_sk, recency, frequency, monetary, sales_per_customer, avg_order_value, effective_date
    FROM rfm_data
{% else %}
    WITH rfm_data AS (
        SELECT 
